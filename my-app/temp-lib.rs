...
pub mod sb_randomness {
    ...
    pub fn coin_flip(ctx: Context<CoinFlip>, randomness_account: Pubkey, guess: u8) -> Result<()> {
        ...
        msg!("User submitted a guess! {}", guess);
    ...
    pub fn settle_flip(ctx: Context<SettleFlip>, escrow_bump: u8) -> Result<()> {
        ...
        msg!("ROLL_RESULT: {}", randomness_result);
        ...
        if randomness_result == player_state.current_guess {
            msg!("You win!");
            let rent = Rent::get()?;
            let needed_lamports = player_state.wager * 52 / 10 + rent.minimum_balance(ctx.accounts.escrow_account.data_len());
            if needed_lamports > ctx.accounts.escrow_account.lamports() {
                ...
                transfer(
                    ...
                    player_state.wager * 52 / 10, // If the player wins, they get 4.2x their wager PLUS original wager
                    seeds // Include seeds
                )?;
            }
        } else {
...
pub struct PlayerState {
    allowed_user: Pubkey,
    latest_flip_result: u8, // Stores the result of the latest roll
